...
```{r}
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting), shape = 0, alpha = .8, size = 3)
```
...
```{r}
ggplot(faithful) + 
  geom_histogram(aes(x = eruptions, fill = eruptions < 3.2), show.legend = F)
```
...
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting)) +
  geom_abline(slope = -13, intercept = 110, color = 'red')
```
...
ggplot(mpg) + 
  ...
  stat_summary(aes(class, hwy, fun = 'mean'), geom = 'point', color = 'red')
```
...
### Answer
`coord_trans()` modify data before plot
`scale_x_continuous()` just modify the scale
